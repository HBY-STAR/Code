...
/*
这里借用了上次lab的商品类，上次用到的链表是自己直接写的，所以这次想着尝试
用STL再重新简单完成一个链表，下面的程序首先依次将ABCDE填入链表，然后借助
反向迭代器反向输出商品名称和价格
*/
...
int main()
{
    int date[3] = {2022, 5, 20};
    int NowDate[3] = {2022, 5, 21};
    list<Product> ProductList;
    Product A = Product("A", 1.0, 1, date, NowDate);
    Product B = Product("B", 2.0, 2, date, NowDate);
    Product C = Product("C", 3.0, 3, date, NowDate);
    Product D = Product("D", 4.0, 4, date, NowDate);
    Product E = Product("E", 5.0, 5, date, NowDate);
    ProductList.push_back(A);
    ProductList.push_back(B);
    ProductList.push_back(C);
    ProductList.push_back(D);
    ProductList.push_back(E);
    list<Product>::reverse_iterator ProductList_rIter;
    for (ProductList_rIter = ProductList.rbegin(); ProductList_rIter != ProductList.rend(); ProductList_rIter++)
    {
        string tempstr = ProductList_rIter->GetName();
        double tempprice = ProductList_rIter->GetPrice();
        cout << tempstr << ":" << tempprice << endl;
    }
    system("pause");
    return 0;
}
